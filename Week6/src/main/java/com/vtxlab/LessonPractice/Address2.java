package com.vtxlab.LessonPractice;
...
public class Address2 {
  ...
  public boolean isAddressLineValid() {
   ...
   //}
    ...
    Predicate<String> isAddressLineValid = e -> {
      if (e == null || e.length() <= MAX_ADDRESS_LINE_LENGTH) return true;
      return false;
    };
  ...
  public static void main(String[] args) {
    ...
    if (address.isAddressLineValid()) address.printStatus();
    if (addressForTest.isAddressLineValid()) address.printStatus();
    if (address1.isAddressLineValid()) address.printStatus();
